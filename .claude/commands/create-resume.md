...
4. **Mixed-model execution**: the Challenges, Relevant Experiences, and Fit Assessment steps run on `claude-sonnet-4-6` — delegate all three together as a **single** Agent tool call with `model: claude-sonnet-4-6` (or the `resume-sonnet-4-6` agent type), after Phase 0 has already been written to the job description file. Give the subagent the file paths (job description file, `input/resume.md`, `positioning/differentiators.md` if it exists) and all three steps' instructions in one prompt — the subagent has Read/Write/Edit tools, so it should read each file once, then run Steps 1, 2, and 3 in sequence itself, writing each step's output directly to the job description file (append, never overwrite existing content) as soon as that step completes, before moving to the next. This satisfies the incremental-append requirement below without the orchestrating session re-reading `resume.md` and `differentiators.md` from scratch on three separate cold-start calls. Only delegate as three separate calls if a step needs to be rerun in isolation (e.g. correcting one step after the others already ran). Phase 0 and the Personalised Summary, Pros and Cons, 3 Alternatives, and Application Form Questions steps run inline on the current session model — do not delegate those.
...
**Append incrementally, after each step, not all at once at the end.** The Challenges, Relevant Experiences, and Fit Assessment steps are delegated together as a single subagent call (see Setup point 4) that reads the job description file directly rather than having content pasted into its prompt — the subagent itself writes each of the three steps' output to the file as soon as that step completes, before moving to the next, since Step 2 needs Step 1's challenges already in the file and Step 3 needs Step 2's Gaps-to-flag section already in the file. For Steps 4 onward (run inline, not delegated), write each step's output to the file as soon as that step completes.
...
**Part of the combined Steps 1–3 delegated call (see Setup point 4).** The subagent Reads the job description file directly — by this point it already contains the JD and Phase 0's Company Research, appended — then writes this step's output to the file itself before moving on to Step 2 within the same call.
...
**Part of the combined Steps 1–3 delegated call (see Setup point 4).** The subagent already has `input/resume.md`, the job description file (which now also contains Step 1's 3 challenges, just written), and `positioning/differentiators.md` open from the start of the call — it writes this step's output to the file itself before moving on to Step 3 within the same call.
...
**Final step of the combined Steps 1–3 delegated call (see Setup point 4).** The subagent already has the job description file open — by this point it contains the JD, Phase 0's Company Research, Step 1's challenges, and Step 2's Gaps to flag section (with core/preferred tags) — plus `positioning/differentiators.md`. It writes this step's output to the file itself, then returns a final report (business model verdict, rating, fit basis, reasoning bullets) to the orchestrating session so the Personalised Summary step can proceed.
...
- **Default each Sentence 2 clause to the exact bullet the Relevant Experiences step's Format section cited for that challenge — do not silently substitute a different bullet or company.** Pulling in a bullet the Relevant Experiences step never cited for that challenge (even if it's a real, accurate resume.md fact) is evidence drift: it bypasses the Direct/Bridge check and the SMB/enterprise, GTM, and multi-product-line precision rules already applied upstream. If a genuinely stronger, more specific match exists elsewhere in resume.md that the Relevant Experiences step missed, it's fine to use it — but say so explicitly in the output (e.g. "using [bullet] instead of the Relevant Experiences step's citation because [reason]"), the same way a Direct/Bridge tag makes a citation's nature visible. A silent swap is the failure mode to avoid, not deviation itself.
- **When substituting or diversifying company sourcing across the 3 clauses (e.g. because the Relevant Experiences step's citations repeat a company across two challenges), re-run the same full-resume.md scan Step 2 requires — do not just pick from the companies already cited elsewhere in this run.** The companies already in view (from Step 2's citations, or from earlier challenges' clauses) are not the complete candidate set. Check every company section in resume.md, including ones not yet cited anywhere in this run, for the closest mechanism/audience match before substituting — and don't let a bullet that merely echoes the JD's phrasing win over one with a more precise mechanism/audience match just because the phrase echo is easier to spot.
