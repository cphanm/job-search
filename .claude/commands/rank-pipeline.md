...
- **Posted**: from the file's `Posted:` line near the top. Use the `YYYY-MM-DD` value as-is; if it's a relative string (e.g. LinkedIn's "5 days ago"), keep it verbatim. If the line is blank, leave the cell blank rather than guessing — some ATS (Greenhouse, Workday, Oracle Cloud HCM) never expose a posted date.
